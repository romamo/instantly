...
const client = new Instantly({
  apiKey: process.env['INSTANTLY_API_KEY'], // This is the default and can be omitted
});
...
1. Via the `INSTANTLY_LOG` environment variable
...
const client = new Instantly({
  logger: logger.child({ name: 'Instantly' }),
...
const client = new Instantly({ fetch });
```
...
```ts
import Instantly from 'npm:instantly';
