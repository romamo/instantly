...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['INSTANTLY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['INSTANTLY_LOG'] or 'warn' if it isn't set.
   */
...
export class Instantly {
   ...
   */
  constructor({
    baseURL = readEnv('INSTANTLY_BASE_URL'),
    apiKey = readEnv('INSTANTLY_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.InstantlyError(
        "The INSTANTLY_API_KEY environment variable is missing or empty; either provide it, or instantiate the Instantly client with an apiKey option, like new Instantly({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Instantly.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('INSTANTLY_LOG'), "process.env['INSTANTLY_LOG']", this) ??
      defaultLogLevel;
  ...
  static Instantly = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static InstantlyError = Errors.InstantlyError;
